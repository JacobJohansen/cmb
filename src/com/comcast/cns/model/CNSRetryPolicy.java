...
public class CNSRetryPolicy {
	 ...
	 */
	public CNSRetryPolicy(JSONObject json) throws Exception  {
		
		try {
			sharedJsonUpdate(json);
		} catch (JSONException e) {
			logger.error("event=construct_cns_retry_policy", e);
			throw new CNSModelConstructionException("JSON parameter format error");
		} catch (CNSModelConstructionException e) {
			logger.error("event=construct_cns_retry_policy message=" + e.getErrormessage());
			throw new CNSModelConstructionException(e.getErrormessage());
		}
	}
	...
	public void update(JSONObject json) throws Exception {
		...
		try {
			...
			logger.error("event=update_cns_retry_policy message=" + e.getErrormessage());
			throw new CNSModelConstructionException(e.getErrormessage());
		}
	}


	private void sharedJsonUpdate(JSONObject json) throws Exception {
		boolean error = false;
		String message = "";
